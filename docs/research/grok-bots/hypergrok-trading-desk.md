...
- **Read/write plane separation is structural, not just role text**: only the Execution Trader's Bot holds `writes_to_exchange: true`; every other Bot is read-only against `/info` and public web (`docs/ARCHITECTURE.md:52-56`).
