...
//!
//! The module is split by what a seat is doing: this file holds the prompt
//! assembly and the matched-budget poll shared by every backend, [`agent`]
//! drives one seat as a standalone participant over a CLI subprocess, and
//! [`desk`] drives one seat as a member of a federation of desks that can
//! only reach each other by a referral.
...
use agent::split_command;

pub(crate) use agent::{LiveAgent, marker_line};
pub(crate) use desk::{CROSS_PROTOCOL, CROSS_RULES, LiveDeskAgent};
...
/// seam [`LiveDeskAgent::directory`](desk::LiveDeskAgent) uses for the
/// federation's own move, so a participant reads it alongside the ordinary
/// moves rather than as an afterthought. Taking `lines` as prose rather than
/// the library's own directory type keeps this module free of a dependency on
/// it; the caller wires `Directory::lines()` in.
#[must_use]
    ...
    for agent in &scenario.agents {
        ...
        let text = match backend {
            Backend::Cli => {
                ...
                agent::plain(&String::from_utf8_lossy(&output.stdout))
            }
            Backend::Http { config } => {
