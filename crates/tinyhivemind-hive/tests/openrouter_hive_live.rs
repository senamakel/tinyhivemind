        ...
        let transcript = visible
            ...
            .map(|message| {
                let author = match &message.author {
                    tinyhivemind_hive::SessionAuthor::Agent { label, .. }
                    | tinyhivemind_hive::SessionAuthor::Person { label, .. }
                    | tinyhivemind_hive::SessionAuthor::System { label, .. } => label.as_str(),
                    tinyhivemind_hive::SessionAuthor::Operator => "operator",
                };
...
fn live_agents_deliberate_and_the_episode_terminates_within_its_budget() -> Result<(), String> {
    ...
    let traces = tinyhivemind_hive::read(harness.journal());
    assert!(
        !traces.is_empty(),
